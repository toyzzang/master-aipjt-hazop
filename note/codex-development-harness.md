...
2. `main`은 프로젝트 루트 `/Users/zzang/Desktop/me/project/master-aipjt`에 고정한다.
3. 새 작업은 `.worktrees/작업명` 아래에 별도 worktree를 만들고 그 안에서만 수정한다.
   - 새 기능: `git worktree add .worktrees/작업명 -b feature/작업명`
   - 기존 브랜치: `git worktree add .worktrees/작업명 feature/작업명`
4. 기능 개발은 `feature/작업명`, 버그 수정은 `bug/작업명` 형식의 별도 브랜치를 만든다.
5. 무엇을 왜 바꿀지 설계를 먼저 파일로 남긴다.
6. 설계에 포함된 성공 기준을 확인한다.
7. 설계 범위 안에서만 소스를 수정한다.
8. 샘플 시나리오 기준으로 다시 테스트한다.
9. 테스트 결과와 남은 리스크를 사용자에게 보고한다.

## 1.1 PR 이후 정리 절차

사용자가 "정리해줘"라고 요청하면 아래 순서를 지킨다.

1. 현재 작업 worktree 경로와 브랜치를 확인한다.
2. 작업 브랜치가 `origin/브랜치명`에 push되어 있는지 확인한다.
3. 해당 브랜치에서 대상 브랜치, 기본값 `main`, 로 열린 PR이 있는지 확인한다.
4. PR이 없거나 push되지 않은 변경이 있으면 worktree/브랜치를 삭제하지 않고 사용자에게 상태를 보고한다.
5. PR이 확인되면 작업 worktree를 삭제한다.
   - 예: `git worktree remove .worktrees/작업명`
6. worktree 삭제 후 로컬 브랜치를 삭제한다.
   - 예: `git branch -d feature/작업명`
7. 원격 브랜치는 PR이 아직 열려 있으면 유지한다.
8. PR이 merge/close 되었고 사용자가 원격 브랜치 삭제를 요청한 경우에만 원격 브랜치를 삭제한다.
   - 예: `git push origin --delete feature/작업명`
9. 정리 결과로 삭제한 worktree, 삭제한 로컬 브랜치, 남겨둔 원격 브랜치/PR 상태를 보고한다.
