...
현재 KOSHA 연동은 JSON Open API가 아니라 KOSHA 검색 화면에 form POST를 보내는 사이트 내부 검색입니다.
검색 목록 HTML에서 `chem_id`와 물질명을 찾고, 상세 HTML에서 H문구(유해·위험문구)와
P문구(예방조치문구)를 추출한 뒤 아래 내부 구조로 정리합니다.

```json
{
  "material": "Ammonia",
  "hazards": ["H220 : 극인화성 가스", "H280 : 고압가스"],
  "handling": ["P210 : 점화원으로부터 멀리하시오"],
  "source": "KOSHA MSDS 검색 chem_id=001174",
  "is_high_hazard": true,
  "hazard_signals": ["고위험 H문구: H220, H280", "화재/폭발"]
}
```

KOSHA가 `is_high_hazard` 값을 직접 주는 것은 아닙니다. 시스템이 H문구와 독성·부식성·화재·폭발
키워드를 보수적인 규칙으로 분류하며, 판정 근거는 `hazard_signals`에 함께 남깁니다.

## PoC 사고·정비 이력 조회

사고·정비 이력은 별도 DB 없이 아래 JSON 파일을 로컬 저장소처럼 실제 검색합니다.

```text
app/data/incident_history/incident_history_samples.json
```

Workflow가 입력된 Maker, Model, 물질, Node, 변수, Guideword를 검색어로 조합하고,
일치도가 높은 사고·Near Miss·Alarm·정비 이력을 최대 5건 찾아 Agent 공통 Context에 넣습니다.
Agent가 `lookup_incident_history` Tool을 추가 호출할 때도 같은 JSON 파일을 조회합니다.

이 데이터는 시연용 `PoC 합성 샘플`이며 실제 사업장 사고 통계가 아닙니다.
`frequency_hint`도 최종 빈도 확정값이 아니라 위험도 기준표와 함께 검토하는 참고값입니다.
...
Agent의 구조화 실행 로그는 서버를 실행한 같은 터미널에서 확인할 수 있습니다.

```text
AGENT_TRACE {"request_id":"...","agent_id":"risk-review-agent","kind":"self-correction",...}
```

Docker를 사용하면 API 서비스 이름을 확인한 뒤 로그를 따라갑니다.
