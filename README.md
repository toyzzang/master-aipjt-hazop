...
```text
Streamlit 2분할 화면: http://127.0.0.1:8501
기존 기본 frontend: http://127.0.0.1:8000
```

Docker Compose는 Streamlit 화면과 기존 기본 frontend를 함께 실행합니다.
...
Streamlit 2분할 화면으로 실행할 때는 아래 명령을 씁니다.
...
기존 FastAPI 화면과 API를 로컬에서 따로 실행할 때는 아래 명령을 씁니다.
...
2. `http://127.0.0.1:8501` 접속
3. `samples/HAZOP_CleanTech_CT-DIW-100.xlsx` 업로드
4. Maker/Model/물질 기본값 확인
5. `AI 초안생성` 클릭
6. Agent 로그가 실시간으로 쌓이는지 확인
7. `#3 위험성평가` 테이블이 생성되는지 확인
8. `결과 Excel 다운로드` 클릭
